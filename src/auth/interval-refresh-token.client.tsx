...
export default function IntervalRefreshToken({ accessTokenExp, refreshToken }: { accessTokenExp: number, refreshToken: string }) {
    useEffect(() => {
        // 检查并刷新 accessToken
        const refresh = async () => {
            console.log('refresh token');
            if(Date.now() / 1000 > accessTokenExp - TIME_OFFSET) {
